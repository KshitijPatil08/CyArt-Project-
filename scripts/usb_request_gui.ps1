...
$submitButton.Add_Click({
        ...
        try {
            ...
            $path = "/api/usb/request"
            $signature = ""
            
            if ($script:AuthToken) {
                $sha256 = New-Object System.Security.Cryptography.HMACSHA256
                $sha256.Key = [System.Text.Encoding]::UTF8.GetBytes($script:AuthToken)
            ...
            $headers = @{
                ...
                "X-CyArt-Timestamp" = $timestamp
            }
            if ($signature) {
                $headers.Add("X-CyArt-Signature", $signature)
            }

            $response = Invoke-RestMethod -Uri $script:API_URL -Method Post -Body $jsonPayload -Headers $headers
