...
var (
	...
	// Track last block state to avoid redundant PnP calls
	lastStorageBlockedState = false
...
func checkPolicies() {
	...
	wasBlocked := lastStorageBlockedState // NEW: Read current blocked state
	...
	// === PRIORITY DEFINITION (Safest Wins) ===
	// 1. Quarantine (Absolute)
	// 2. Expiration Date (Block)
	// 3. Device Disablement / Active=False (Block)
	// 4. Time Windows (Block)
	// 5. Data Limits (Forced Read-Only)
	// 6. Manual Read-Only (Read-Only)
	...
	if !shouldBlock && gExpiration != "" {
		...
		expiryDate := normalizeDate(gExpiration)

		if expiryDate != "" && today > expiryDate {
			shouldBlock = true
			blockReason = "Global USB Access Expired on " + expiryDate
		}
	}
	...
	if !shouldBlock && gDataLimit > 0 && currentGlobalUsage >= gDataLimit {
		shouldReadOnly = true
		// We don't block, we just enforce Read-Only
	...
	for _, serial := range connectedSerials {
		...
		if policy.ExpirationDate != "" {
			today := time.Now().Format("2006-01-02")
			expiryDate := normalizeDate(policy.ExpirationDate)

			if expiryDate != "" && today > expiryDate {
				shouldBlock = true
				blockReason = fmt.Sprintf("Device %s Access Expired on %s", serial, expiryDate)
				break
			}
		}
		...
		if policy.MaxDailyTransferMB > 0 {
			...
			if deviceUsage >= policy.MaxDailyTransferMB {
				...
				// break
				// Don't break, continue to check other policies or let Read-Only apply
			}
		}
	...
	if shouldBlock {
		if !wasBlocked {
			policyMutex.Lock()
			if lastStorageBlockedState { // Double-check inside lock
				policyMutex.Unlock()
				return
			}
			lastStorageBlockedState = true
			policyMutex.Unlock()
		...
		if !wasRO {
			policyMutex.Lock()
			if lastReadOnlyState { // Double check inside lock
				policyMutex.Unlock()
				return
			}
			lastReadOnlyState = true // Mark it early to prevent concurrent cycles
			policyMutex.Unlock()
		...
		if wasRO {
			...
			lastReadOnlyState = false // Mark early
			policyMutex.Unlock()
		...
		if wasBlocked {
			...
			logMessage("🔓 Restoring USB Access (Unblocking Services)")
			unblockUSBStorage()
		}
	}
}
...
// normalizeDate extracts YYYY-MM-DD from various date string formats
func normalizeDate(dateStr string) string {
	if dateStr == "" {
		return ""
	}
	// Handle ISO8601 (T separator)
	...
	if len(parts) == 3 {
		// Ensure leading zeros for month/day
		year := parts[0]
		month := parts[1]
		day := parts[2]
		if len(month) == 1 {
...
func trackUSBDataUsage() {
	...
	if currentLimit > 0 && currentUsage >= currentLimit {
		...
		sendLog(LogEntry{
			...
			Source:     "agent-policy",
			Severity:   "critical",
			Message:    fmt.Sprintf("Global USB Data Limit Exceeded (%.2f/%.2f MB) - Enforcing Read-Only", currentUsage, currentLimit),
			Timestamp:  time.Now().UTC().Format(time.RFC3339),
		})

		// Trigger immediate enforcement via checkPolicies
		saveAgentState()
		checkPolicies()
	}
...
func trackUSBDevices() {
	...
	// Thread-Safe Quarantine & Device Tracking Access
	policyMutex.RLock()
	quarantined := isQuarantined
	lastConnected := make(map[string]bool)
	for k, v := range lastConnectedUSB {
		lastConnected[k] = v
	}
	policyMutex.RUnlock()
	...
	for _, d := range list {
		...
		if !lastConnected[serial] || shouldForceUpdate {
			if !lastConnected[serial] {
	...
	for serial := range lastConnected {
	...
	for _, file := range files {
		...
		if err == nil {
			...
			if isUnverified {
				...
				if isOld {
					reason = fmt.Sprintf("Outdated Software (Year: %d)", year)
				}
